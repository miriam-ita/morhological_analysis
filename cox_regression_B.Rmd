---
title: "Statistical analysis on morphology evaluation - Adjuvant PACT-15 group"
output:
...
```
The following code tests the statistical validity of the morphological analysis carried out on the pancreatic cancer patients belonging to the adjuvant group (Adj GEM-Adj PEXG) of the PACT-15 clinical trial. It evaluates if one or more covariates affect overall survival (OS) and disease-free survival(DFS).
...
df_survival <- df_survival %>%
  ...
  filter(Treatment == "Adj GEM" |Treatment ==  "Adj PEXG") %>% # filtered adj cases
  filter(AUS_ID != "ID_45", AUS_ID != "ID_55") # remove two excluded cases
...
df_survival <- df_survival %>% 
  ...
  mutate(Treatment = case_when(Treatment == "Adj GEM" ~ "1",
                               Treatment == "Adj PEXG" ~ "2"),
         ...
         Regr._D = case_when(Regr._D == "0" ~ "0",
                           Regr._D == "1" | Regr._D == "2" ~ "1"))
...
df_survival <- df_survival %>%
  mutate(Treatment = factor(Treatment,
                            levels = 1:2,
                            labels = c("Adjuvant GEM", "Adjuvant PEXG")),
         Sex = factor(Sex,
                      levels = 0:1,
         ...
         Site = factor(Site, 
                       levels = 1:5,
                       labels = c("Head", "Body", "Tail", "Head + Ampulla",
                                  "Head + Body")),
...
df_survival$G <- fct_recode(df_survival$G, NULL = "G1")
...
```{r Kaplan Meier curves OS, echo=FALSE, warning=FALSE, out.height="30%", out.width="50%"}
...
cols.factor <- df_survival %>% 
  select_if(is.factor) %>% 
  select(-AUS_ID, -Sex, -Site, -Lymphnodes, -Treatment) %>% 
  names()
...
fd <- coxph(Surv(OS, census_OS) ~ Treatment, data = df_survival) 


# summary(fd)
# ggforest(fd)
# print(fd)
# ggadjustedcurves(fd, variable = "Treatment", data = df_survival)
# cox.zph(fit) %>% plot()
# ggcoxdiagnostics(fd)
# 
# cox_univariate(x = "Treatment") %>% cox_as_data_frame()
# ggadjustedcurves(variable = "Treatment", data = df_survival)
...
# df <- NULL
# for (variable.name in covariates) {
#   # print(variable.name, quote = FALSE)
#   res_coxph <- cox_univariate_OS(x = variable.name)
#   res <- print(cox_as_data_frame(res_coxph))
#   df <- rbind(df, res)
# }

# write_csv(df, here("NGS/data", "res_cox.csv"))
# 
# res_cox <- read.csv(here("NGS/data", "res_cox_modified.csv"))
# df <- res_cox

# cox_univariate_OS(x = "Sex")  # p = 0.38
# cox_univariate_OS(x = "Site") # p = 0.79
# cox_univariate_OS(x = "Grade") # p = 0.21
# cox_univariate_OS(x = "Nodes") # p = 0.5
# cox_univariate_OS(x = "Resection_margin") # p = 0.28
# cox_univariate_OS(x= "Dispersion") # p = 0.044
# cox_univariate_OS(x = "Perineural_invasion")  # p = 0.02
# cox_univariate_OS(x = "Vascular_invasion") # p = 0.89
# cox_univariate_OS(x = "Lymphnodes") # p = 0.41
# cox_univariate_OS(x = "Necrosis") # p = 0.042
# cox_univariate_OS(x = "Vascular_alteration") # p = 0.045
# cox_univariate_OS(x = "PanIN_IPMN") # p = 0.72
# cox_univariate_OS(x = "Keloid") # p = 0.13
# cox_univariate_OS(x = "Hyaline") # p = 0.69
# cox_univariate_OS(x = "Cellular_stroma") # p = 0.83
# cox_univariate_OS(x = "Stroma_Neoplasia_Ratio") # p = 0.88
# cox_univariate_OS(x = "TLS") # p = 0.37
# cox_univariate_OS(x = "Lymphocytes") # p = 0.97
# cox_univariate_OS(x = "Granulocytes") # p = 0.32
# cox_univariate_OS(x = "Macrophages") # p = 0.02
# cox_univariate_OS(x = "Age") # p  0.66
# cox_univariate_OS(x = "LN_Ratio") # p = 0.51
```
...
cox_tab_OS <- tbl_uvregression(df_survival[c("Grade", "Nodes", "Resection_margin",
                               ...
                               "Macrophages", "Mucin", "Duodenal_invasion",
                               "Regressive_changes",
                               "LN_Ratio", "OS", "census_OS")],
                 method = coxph,
                 y = Surv(OS, census_OS),
                 exponentiate = TRUE) %>% 
  bold_labels() %>% 
...
Due to the small number of cases, the variables showing the likelihood ratio < 0.1 were considered statistically significant and included in the multivariate analysis.
```{r Multivariate Cox regression, echo = FALSE, out.height="50%", out.width="100%"}
# I used only the significant variables from the univariate cox regression

fit_coxph <- coxph(Surv(OS, census_OS) ~  + Perineural_invasion + Macrophages +
                     Necrosis, 
                   data = df_survival)
summary(fit_coxph)
for_plot_B_OS <- ggforest(fit_coxph, data = df_survival,
                          fontsize = 1, main = "Hazard ratio of OS",
                          c(0.02, 0.22, 0.35))

jpeg(here("NGS/figures", "for_plot_B_OS.jpeg"), width = 25, height = 10, units = "cm",
     res = 72, type = "windows")
HM <- plot(for_plot_B_OS)
dev.off()
```

Then, I assess the validity of the Cox model.  
First, I test the proportional hazards assumption for each covariate. The function cox.zph() correlates the corresponding set of scaled Schoenfeld residuals with time, to test for independence between residuals and time. Additionally, it performs a global test for the model as a whole.  
Second, I test the influence of the outliers. 
```{r Check Cox model assumptions, echo=FALSE, out.height="40%", out.width="50%"}
zph <- cox.zph(fit_coxph)
zph
plot(zph)
# (see http://www.sthda.com/english/wiki/cox-model-assumptions#testing-proportional-hazards-assumption)

ggcoxdiagnostics(fit_coxph, type = "dfbeta", linear.predictions = FALSE,
                 ggtheme = theme_bw())

ggcoxdiagnostics(fit_coxph, type = "deviance", linear.predictions = FALSE,
                 ggtheme = theme_bw())
```


From the output above, the test for the proportional-hazards assumption is not statistically significant for each of the covariates.In addition, the global test is  not statistically significant, as well. Therefore, we can assume the proportional hazards.

The dfbeta plots show some outliers.  

**Questions:**
Is the pattern of the dfbeta graphs relevant? Is it influencing the statistical significance of the analysis?  

**Conclusions:**  
The analysis shows that the hazard ratio tend to be higher in presence of perineural invasion, necrosis and macrophagic infiltration, even if only the latter reaches statistical significance. 
```
```
---
## Survival analysis based on Disease-free survival  
I repeat the same analysis based on the Disease Free Survival.  
First, I estimate the survival probability for each variable with the Kaplan-Meier curves and the logrank test.  
Then, I evaluate the association between survival time and each variable with the univariate Cox regression model. Statistically significant covariates are included in the multivariate model.
```{r Kaplan-Meyer curves DFS, echo=FALSE, warning=FALSE,out.height="30%", out.width="50%"}
for (variable.name in cols.factor) {
  # print(variable.name, quote = FALSE)
...
# Run for loop to parse all variables
# df_DFS <- NULL
# for (variable.name in covariates) {
#   # print(variable.name, quote = FALSE)
#   res_coxph <- cox_univariate_DFS(x = variable.name)
#   res <- print(cox_as_data_frame(res_coxph))
#   df_DFS <- rbind(df_DFS, res)
# }
...
# cox_univariate_DFS(x = "Sex") # p = 0.79
# cox_univariate_DFS(x = "Site") # p = 0.47
# cox_univariate_DFS(x = "Grade") # p = 0.02
# cox_univariate_DFS(x = "Nodes") # p = 0.29
# cox_univariate_DFS(x = "Resection_margin") # p = 0.23
# cox_univariate_DFS(x= "Dispersion") # p = 0.16
# cox_univariate_DFS(x = "Perineural_invasion") # p = 0.39
# cox_univariate_DFS(x = "Vascular_invasion") # p = 0.75 
# cox_univariate_DFS(x = "Lymphnodes") # p = 0.24
# cox_univariate_DFS(x = "Necrosis") # p = 0.12
# cox_univariate_DFS(x = "Vascular_alteration") # p = 0.32
# cox_univariate_DFS(x = "PanIN_IPMN") # p = 0.27
# cox_univariate_DFS(x = "Keloid") # p = 0.38
# cox_univariate_DFS(x = "Hyaline") # p = 0.91
# cox_univariate_DFS(x = "Cellular_stroma") # p = 0.57
# cox_univariate_DFS(x = "Stroma_Neoplasia_Ratio") # p = 0.96
# cox_univariate_DFS(x = "TLS") # p = 0.72
# cox_univariate_DFS(x = "Lymphocytes") # p = 0.54
# cox_univariate_DFS(x = "Granulocytes") # p = 0.3
# cox_univariate_DFS(x = "Macrophages") # p = 0.042
# cox_univariate_DFS(x = "Age") # p  0.92
# cox_univariate_DFS(x = "LN_Ratio")  # p = 0.38
```
...
cox_tab_DFS <- tbl_uvregression(df_survival[c("Grade", "Nodes", "Resection_margin",
                               ...
                               "Cellular_stroma", "Stroma_Neoplasia_Ratio",
                               "TLS", "Lymphocytes", "Granulocytes",
                               "Macrophages", "LN_Ratio",
                               "Mucin", "Duodenal_invasion",
                               "Regressive_changes", "DFS", "census_event_free")],
                 method = coxph,
...
```{r Cox multivariate regression model DFS, echo=FALSE, out.height="100%", out.width="100%"}
fit_coxph_DFS <- coxph(Surv(DFS, census_event_free) ~ Grade + 
                         Macrophages, 
                       data = df_survival)
summary(fit_coxph_DFS)
for_plot_B_DFS <- ggforest(fit_coxph_DFS, data = df_survival, fontsize = 1,
                           main = "Hazard ratio of DFS",
                           c(0.02, 0.22, 0.33))

jpeg(here("NGS/figures", "for_plot_B_DFS.jpeg"), width = 30, height = 10, units = "cm",
     res = 72, type = "windows")
HM <- plot(for_plot_B_DFS)
dev.off()
```


```{r Cox multivariate regression model diagnostics DFS, echo=FALSE, out.height="40%", out.width="50%"}
zph_DFS <- cox.zph(fit_coxph_DFS)
zph_DFS
plot(zph_DFS)
# (see http://www.sthda.com/english/wiki/cox-model-assumptions#testing-proportional-hazards-assumption)

ggcoxdiagnostics(fit_coxph_DFS, type = "dfbeta", linear.predictions = FALSE,
                 ggtheme = theme_bw())

ggcoxdiagnostics(fit_coxph_DFS, type = "deviance", linear.predictions = FALSE,
                 ggtheme = theme_bw())
```
...
**Conclusions:**  
In the multivariate analysis, G3 grade and the presence of macrophages are negative prognostic factors.